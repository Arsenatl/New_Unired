...
export default async function PaymentsNum() {
    const data = await getData()

    return (
        <section className="container">
            <div className="row">
                <div className="grid grid-cols-4 gap-[30px] mt-24 mb-32">
                    {
                        data.map((element, value) => {
                            return (
                                <div key={value}>
                                    <div className='w-8 h-8 bg-[#F4494966] flex justify-center items-center rounded-[50%]'>
                                        <p className='text-18 font-semibold text-[#F44949]'>{element.id}</p>
                                    </div>
